...
export class SearchComponent implements OnInit {
  ...
  constructor(private ipinfoService: IpinfoService,  private countryCode: CountryCodeService, ngProgress: NgProgress, private route: ActivatedRoute, private router: Router, private titleService: Title, private searchTerm: SearchshareService, private location: Location ) {
      ...
      if (this.ip) {
          const checker = new RegExp('^((([1-9]|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])\\.)((\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])\\.)((\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])\\.)((\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])))$');
          ...
          const checker = new RegExp('(^(?:([^:/\\s]*)\\.)*((?:[^.:/\\s]+)+(?:\\.[^.:/\\d]+){1})(?:\\/.*)*$)');
  ...
  ngOnInit() {
     ...
     this.ipinfoService.getData(this.search_term, this.type).subscribe(results => {
       ...
       const url = 'search/' + this.type + '/' + this.result[this.type];
       ...
       if (this.result.c2 && this.result.c2.feed) {
            const c2 = new RegExp('Master Indicator Feed for (\\w*)');
            const c2_name = c2.exec(this.result.c2.feed);
       ...
       if (this.result.malware && !this.result.malware.count && this.result.malware != 'FA') {
            this.result.malware.count = 0;
       }
       if (this.result.url && !this.result.url.count  && this.result.url != 'FA') {
            this.result.url.count = 0;
       }
       if (this.result.ip && !this.result.ip.count  && this.result.ip != 'FA' && typeof this.result.ip === 'object') {
