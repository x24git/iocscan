...
router.post('/ip', function(req, res) {
		...
		if (terms.ip && ipv4check.test(terms.ip) && !privateip.test(terms.ip)) {
			...
			if (!terms.ip) {
				...
				var error = new Error("IPv4 Private Address Space");
				error.status = 421;
				error.code = "Misdirected Request"
				throw error;
			}
			else if (!ipv4check.test(terms.ip)) {
				var error = new Error("IPv4 Bad Format");
				error.status = 400;
		...
		async function Results() {
			//[REMOVED]
			intel = feed.resultAnalysis({type: 'ip' })
	...
	router.post('/domain', function(req, res) {
		var terms = req.body;
		var domaincheck_level2 = /(?:([^:/]*)\.)*(([^.:/]+)(?:\.)((?:[^.:/]+)(?:\.[^.:/]{2})))(?:\/.*)*$/;
		var domaincheck_level1 = /(?:([^:/]*)\.)*((?:[^.:/]+)+(?:\.[^.:/]+){1})(?:\/.*)*$/;
		if (domaincheck_level2.test(terms.domain))
			var domaincheck = domaincheck_level2;
		else
			var domaincheck = domaincheck_level1;
		var subdomain, domain, check, recover = '';
		if (terms.domain && domaincheck.test(terms.domain)) {
			...
			if (!terms.domain) {
				...
				var error = new Error("Domain Bad Format");
				error.status = 400;
				error.code = "Bad Request"
				throw error;
			}
		...
		async function DomainResults() {
			//[REMOVED]
			var intel = feed.resultAnalysis({type: 'domain' })
			return res.json(intel);
